...
const updateLibraryMedia = async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      ...
      if (title) libraryMedia.title = title;
      if (desc) libraryMedia.desc = desc;
      if (url) libraryMedia.url = url;
      if (mediaType) {
        if (mediaType !== 'audio' && mediaType !== 'video') throw new ApiError('invalid Media type', 400);
        libraryMedia.mediaType = mediaType;
      }

      if (thumbnail) libraryMedia.thumbnail = { url: thumbnail, provider: 'external' };
      if (req.file) libraryMedia.thumbnail = { url: req.file.path, provider: 'internal' };
