...
const updateLiveDarshan = async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      ...
      if (titleEnglish) liveDarshan.titleEnglish = titleEnglish.trim();
      if (titleHindi) liveDarshan.titleHindi = titleHindi.trim();
      if (descEnglish) liveDarshan.descEnglish = descEnglish.trim();
      if (descHindi) liveDarshan.descHindi = descHindi.trim();
      if (location) liveDarshan.location = location.trim();
      if (url) liveDarshan.url = url.trim();
      if (thumbnail)
        liveDarshan.thumbnail = { url: thumbnail, provider: "external" };
      if (!thumbnail && url) {
        const v_id = new URL(url).searchParams.get("v");
        liveDarshan.thumbnail = {
          url: `https://img.youtube.com/vi/${v_id}/0.jpg`,
          provider: "external",
        };
      }
      if (req.file)
        liveDarshan.thumbnail = { url: req.file.path, provider: "internal" };
      if (tags) liveDarshan.tags = String(tags).split(",");
      if (isLive) {
        if (isLive !== "true" && isLive !== "false")
          throw new ApiError("Invalid isLive value", 400);
        liveDarshan.isLive = isLive === "true" ? true : false;
      }
      if (isBroken) {
        if (isBroken !== "true" && isBroken !== "false")
          throw new ApiError("Invalid isBroken value", 400);
